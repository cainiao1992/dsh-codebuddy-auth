...
/**
 * The CLI client identity the chat plane and the login OAuth expect.
 * DEFAULTS = China edition (endpoints + identity).
 */
...
/**
 * The VSCode-shaped identity the craft-agent catalog is served under
 * (X-Agent-Intent: craft + X-IDE-* headers). The chat plane uses the CLI
 * identity from DEFAULTS; the catalog plane uses this. fetchRemoteModels is
 * the only consumer; kept separate from DEFAULTS so the two planes'
 * identities cannot be mistaken for each other.
 */
...
/** GET /v3/config — craft-agent model discovery. The `X-Agent-Intent: craft`
 *  header makes the service answer as the craft agent, whose model list this
 *  plugin serves; the VSCode-shaped identity (CATALOG_IDENTITY) rides along
 *  because it is what the craft agent's catalog expects. Without it the
 *  endpoint still answers (any UA works) but returns the non-craft catalog. */
export async function fetchRemoteModels(accessToken, cfg = DEFAULTS) {
  const headers = {
    ...
    'User-Agent': `${CATALOG_IDENTITY.ideName}/${CATALOG_IDENTITY.ideVersion} CodeBuddy/${cfg.appVersion}`,
    'X-Agent-Intent': cfg.agentIntent,
    'X-IDE-Type': CATALOG_IDENTITY.ideType,
    'X-IDE-Name': CATALOG_IDENTITY.ideName,
    'X-IDE-Version': CATALOG_IDENTITY.ideVersion,
  ...
  // The VSCode-shaped identity above makes /v3/config answer as the `craft`
  // agent, whose model list is the one this plugin serves. If the agent is
  // ever missing, fall back to every tool-call-capable model rather than
  // silently offering an empty catalog.
  const craft = (body.data.agents || []).find((a) => a.name === cfg.agentIntent);
  const ids = (craft && craft.models) || allModels.map((m) => m.id);
