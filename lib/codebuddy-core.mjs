...
/**
 * Build the `reasoningEfforts` field for one model, given its `/v3/config`
 * `reasoning` metadata and a reasoning config.
 *
 * The config is one of:
 *   - `'auto'` (default): mirror the model's own capability — its
 *     `supportedEfforts` become selectable levels, and `canDisableThinking`
 *     adds an `off` (disable) level; a fixed `effort` becomes a single level.
 *   - `'off'`: declare the model as non-reasoning (`false`).
 *   - a thinking level like `'low'` | `'high'`: force that single level.
 *
 ...
 */
export function buildReasoningEfforts(m, reasoningConfig = 'auto') {
  if (reasoningConfig === 'off') return false;
  if (reasoningConfig && reasoningConfig !== 'auto') {
    // A single forced level: map it only if the model offers it.
  ...
  if (r.effort) {
    const level = CB_EFFORT_TO_LEVEL[r.effort];
    if (level) efforts[level] = r.effort;
  }
...
export function remoteModelToProfile(m, reasoningConfig = 'auto') {
  const entry = {
    id: m.id,
    name: m.name || m.id,
  ...
  const efforts = buildReasoningEfforts(m, reasoningConfig);
  if (efforts !== false) {
