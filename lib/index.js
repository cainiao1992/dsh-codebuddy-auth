...
export function apply(ctx) {
   ...
   * login / refresh / model-discovery endpoints all follow. Matches the
   * upstream plugin: detecting `codebuddy.ai` selects the international
   * endpoints, anything else keeps the China-edition defaults.
   */
  function resolveRegion() {
    try {
      const section = ctx.settings.get(NS);
      const route = section && section.providers ? section.providers.codebuddy : undefined;
      const base = route && typeof route.baseURL === 'string' ? route.baseURL : '';
      if (base.includes('codebuddy.ai')) {
        return { ...DEFAULTS, serverUrl: 'https://www.codebuddy.ai', domain: 'www.codebuddy.ai' };
      }
    } catch {
  ...
  async function syncRoute(access, { withModels = true } = {}) {
    ...
    // Region: if the route already points at the international API, make sure
    // X-Domain follows it (the default template uses the China edition).
    try {
      const section = ctx.settings.get(NS);
      const route = section && section.providers ? section.providers.codebuddy : undefined;
      if (route && typeof route.baseURL === 'string' && route.baseURL.includes('codebuddy.ai')) {
        ops.push({ op: 'set', path: [...ROUTE, 'headers', 'X-Domain'], value: 'www.codebuddy.ai' });
      }
    } catch {
      // ignore region-detection errors; identity headers still get written
    }
