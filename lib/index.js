  ...
  async function syncRoute(access, { withModels = true } = {}) {
    ...
    if (withModels) {
      try {
        ...
        if (models.length > 0) {
          const reasoningConfig = (route && typeof route.reasoning === 'string' && route.reasoning)
            ? route.reasoning
            : 'auto';
          ops.push({ op: 'set', path: [...ROUTE, 'models'], value: models.map((m) => remoteModelToProfile(m, reasoningConfig)) });
