...
/**
 * The route this plugin owns, created on first startup so installation is
 * npm-install + one patch row and nothing else: after the next dsh start the
 * provider already shows in the Models page (unauthenticated until login).
 * A minimal placeholder model list rides along because a route the pi-ai
 * catalog does not ship must declare its models outright; login replaces it
 * with the account's real /v3/config list. Never overwrite an existing route.
 */
const ROUTE_TEMPLATE = buildRouteTemplate('cn');
...
export function apply(ctx) {
  ...
  /**
   * Resolve the region's core config from the chat route's baseURL, so a user
   * switches editions by editing one value (settings.yaml `baseURL`) and the
   * login / refresh / model-discovery endpoints all follow. Region values
   * live in EDITIONS; this just picks the one the baseURL targets.
   */
  function resolveRegion() {
    try {
      const section = ctx.settings.get(NS);
      const route = section && section.providers ? section.providers.codebuddy : undefined;
      const edition = editionFromBaseURL(route && route.baseURL);
      return edition === 'intl' ? { ...DEFAULTS, ...EDITIONS.intl } : DEFAULTS;
    } catch {
      // fall through to China edition on any settings-read error
    }
    return DEFAULTS;
  }
  ...
  /** Create the route on first contact; a no-op once it exists. */
  async function ensureRoute() {
    const section = ctx.settings.get(NS);
    if (section && section.providers && section.providers.codebuddy) return false;
    await ctx.settings.mutate(NS, [{ op: 'set', path: ROUTE, value: ROUTE_TEMPLATE }]);
    return true;
  }
  ...
  async function syncRoute(access, { withModels = true } = {}) {
    ...
    // Keep X-Domain consistent with the route's baseURL: whenever it points at
    // the international API, X-Domain must be the international value (the
    // template defaults to the China edition). Centralized through EDITIONS.
    let section;
    try {
    ...
    const route = section && section.providers ? section.providers.codebuddy : undefined;
    const edition = editionFromBaseURL(route && route.baseURL);
    const domain = EDITIONS[edition].domain;
    if (!route || !route.headers || route.headers['X-Domain'] !== domain) {
      ops.push({ op: 'set', path: [...ROUTE, 'headers', 'X-Domain'], value: domain });
    }
    // Keep the spoofed client-version headers in step with this plugin's own
    // constants: a route created by an older plugin version keeps its old
    // X-IDE-Version / X-Product-Version forever otherwise (ensureRoute never
    // overwrites an existing route), so every sync stamps the current values.
    const cfg = resolveRegion();
    if (!route || !route.headers || route.headers['X-IDE-Version'] !== cfg.ideVersion) {
      ops.push({ op: 'set', path: [...ROUTE, 'headers', 'X-IDE-Version'], value: cfg.ideVersion });
    }
    if (!route || !route.headers || route.headers['X-Product-Version'] !== cfg.appVersion) {
      ops.push({ op: 'set', path: [...ROUTE, 'headers', 'X-Product-Version'], value: cfg.appVersion });
    }
    let modelNote = 'models untouched';
    if (withModels) {
      try {
        const models = await fetchRemoteModels(access, cfg);
        if (models.length > 0) {
          // Every model is declared with its full real reasoning capability
          // (selectable levels + off where the model allows disabling). The
          // request *default* level is not our concern: DSH's native route
          // `reasoning` field sets it via the adapter, and unset it falls back
          // to each model's own server-side default.
  ...
  async function doRefresh() {
    ...
    const tokens = await refreshAccessToken(refresh, resolveRegion());
  ...
  // fresh model list without waiting for a manual tool call — and materialize
  // the route itself on first boot, so a fresh install already shows CodeBuddy
  // in the Models page (unauthenticated) before any login happens.
  void (async () => {
    if (!live) return;
    try {
      if (await ensureRoute()) ctx.logger.info('codebuddy-auth: created the llm-pi-ai codebuddy route (log in to populate models)');
      const access = await readCredential(ACCESS_REF);
      const refresh = await readCredential(REFRESH_REF);
      const expiresAt = access ? tokenExpiresAt(access) : undefined;
