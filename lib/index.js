...
export function apply(ctx) {
  ...
  function resolveRegion() {
    try {
      ...
      return cfgForEdition(editionFromBaseURL(route && route.baseURL));
    } catch {
      // fall through to China edition on any settings-read error
    }
    return cfgForEdition('cn');
  }
  ...
  async function syncRoute(access, { withModels = true } = {}) {
    ...
    // Strip the IDE identity block earlier versions stamped onto the chat
    // plane: chat requests need identity only (see CHAT_IDENTITY_KEYS /
    // buildRouteTemplate), and ensureRoute never touches an existing route,
    // so migration happens here — unset each legacy key once it's present.
    if (route && route.headers) {
      for (const key of LEGACY_CHAT_HEADERS) {
        if (route.headers[key] !== undefined) {
          ops.push({ op: 'unset', path: [...ROUTE, 'headers', key] });
        }
      }
    }
    ...
    if (withModels) {
      try {
        const models = await fetchRemoteModels(access, resolveRegion());
        if (models.length > 0) {
