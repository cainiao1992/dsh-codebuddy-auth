 ...
 * Registers a `codebuddy` adapter on ctx.llm (own chat wire: minimal identity
 * headers, CLI UA, own SSE + serialization — ported from shatyuka/dsh-llm-
 * codebuddy, MIT), plus the `codebuddy` tool (login/status/refresh/logout/
 * sync-models), credential storage + auto-refresh, and editions.
 */
...
export async function apply(ctx, config = {}) {
  ...
  // Remove the legacy llm-pi-ai settings route (pre-0.7 installs). The
  // native adapter owns the provider; a stale settings route would make
  // registerAdapter throw DUPLICATE_ADAPTER. No-op when there is nothing to
  // migrate. Callers await it BEFORE registering.
  ...
  // The pi-ai adapter watches settings and releases the route asynchronously
  // after our unset, so a brief race remains: retry a few times, then degrade
  // to a warning rather than failing the whole plugin tree.
  ...
  for (let attempt = 1; attempt <= 3 && !registered; attempt++) {
    try {
      ...
      if (attempt === 3) {
        ctx.logger.warn('codebuddy-auth: another adapter already serves the codebuddy provider; not registering ours');
      } else {
        await sleep(150);
      }
    }
  ...
  // Startup: keep a usable access token and warm the catalog; report state.
  void (async () => {
    if (!live) return;
    try {
      const access = await readCredential(ACCESS_REF);
  ...
  // Mid-process expiry guard: rotate whenever less than an hour of validity
  // remains, so a long-lived dsh never hits a stale token mid-request.
